        ...
        return (
            <>
                <View style={{ width: "100%", }}>
                    <UserChips nombre={user.nombre} email={user.email} puntos={user.puntos} userRank={userRank} />
                </View>
            </>
        );
    };

    const renderItem = ({ item, index }: { item: UsuarioRankingDTO; index: number }) => (
        <View style={styles.card}>
            <View style={styles.rankCircle}>
                <Text style={styles.rankText}>{index + 4}</Text>
            </View>
            <Ionicons style={styles.userImage} name="person-circle" size={50} color="grey" />
        ...
        return (
            <View style={styles.podiumContainer}>
                {topUsuarios.slice(0, 3).map((item, index) => (
                    <View key={index} style={[styles.podiumCard, { backgroundColor: podiumColors[index] }]}>
                        <Ionicons name="person-circle" size={60} color="#fff" />
                        <Text style={styles.podiumName}>{item.nombre}</Text>
                        <Text style={styles.podiumPoints}>Puntos: {item.puntos}</Text>
