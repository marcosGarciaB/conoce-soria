...
const SearchScreen = ({ navigation }: { navigation: any }) => {
    ...
    const buttonFilter = (categoria: string) => {
        ...
        const filtered = all.filter(exp => {
            const matchCat = newSelected ? exp.categoria.toUpperCase() === newSelected.toUpperCase() : true;
            const matchText = exp.titulo.toLowerCase().includes(searchText.toLowerCase());
    ...
    const wordsFilter = (texto: string) => {
        ...
        const filter = all.filter(exp => {
            const matchCat = selectedCat ? exp.categoria.toUpperCase() === selectedCat.toUpperCase() : true;
            return matchCat && exp.titulo.toLowerCase().includes(texto.toLowerCase());
        });
            ...
            style={styles.card}>
                ...
                <View style={styles.categoryBadge}>
                    <Text style={styles.categoryText}>
                        {item.categoria.replace('_', ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase())}
                    </Text>
                </View>
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.headerPanel}>
                ...
                <Ionicons name={"search-outline"} size={30} color={"orange"} />
            </View>
