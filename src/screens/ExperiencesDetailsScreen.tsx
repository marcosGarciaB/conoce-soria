...
import { experienciaService, ExperienciaDetailResponse } from '../services/experienceService';
import { comentariosService, ComentariosResponse } from '../services/commentService';
...
const DetailsScreen = ({ navigation, route }: { navigation: any, route: DetailsRoute }) => {
    ...
    // Cargar detalles y los comentarios
    useEffect(() => {
        ...
        const loadComentarios = async () => {
            try {
                const data = await comentariosService.getComentarios((experiencia.id).toString());
                setComentarios(data);
            } catch (error) {
                console.error('Error cargando experiencias:', error);
            }
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            ...
            await comentariosService.setComentario((experiencia.id).toString(), {
                texto: data.comentario
            }, token);

            const updatedComentarios = await comentariosService.getComentarios((experiencia.id).toString());
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.headerPanel}>
                <Ionicons style={{ marginLeft: 15 }} name="chevron-back-circle" size={30} color="grey" onPress={() => navigation.goBack()} />
                <Text style={styles.title}>{detalle.titulo}</Text>
                <Ionicons style={{ marginRight: 15 }} name="location" size={30} color="grey" />
            </View>
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }}
                ...
                <View style={styles.sectionTitleRow}>
                    ...
                    <Text style={styles.sectionTitle}>Meter resto de fotos</Text>
                ...
                <View>
                    <TouchableOpacity style={styles.inputButton} onPress={() => console.log('Botón presionado')}>
                        <Ionicons name="qr-code" size={30} color="white" style={{ marginRight: 10 }} />
