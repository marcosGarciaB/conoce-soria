...
import {
    View, Text, TextInput, TouchableOpacity, StyleSheet, LayoutAnimation, Platform, UIManager,
} from 'react-native';
...
const Filters: React.FC<FiltersProps> = ({ searchText, setSearchText, selectedCat, setSelectedCat, categories, onFilterByText, onFilterByCategory }) => {    // Comprobar si está logueado
    ...
    return (
        <View style={styles.container}>
            <TouchableOpacity style={styles.button} onPress={() => toggleOpen()}>
                <Text style={styles.buttonText}>{open ? "Cerrar Filtros" : "Abrir Filtros"}</Text>
                <Ionicons name={open ? 'chevron-up' : 'chevron-down'} size={20} color="black" />
            </TouchableOpacity>
                ...
                <View style={styles.filtersContainer}>
                    ...
                    <View style={styles.buttonsContainer}>
                        {categories.map(cat => (
                            <TouchableOpacity
                                key={cat}
                                style={[styles.button, selectedCat === cat && styles.buttonCategorySelected]}
                                onPress={() => handleCategory(cat)}
                            >
                                <Text style={[styles.buttonText, selectedCat === cat && styles.buttonTextCategorySelected]}>
