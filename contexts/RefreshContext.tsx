...
export const RefreshProvider = ({ children }: { children: ReactNode }) => {
	const [passportListeners, setPassportListeners] = useState<Set<() => void>>(
		new Set()
	);
	const [topListeners, setTopListeners] = useState<Set<() => void>>(
		new Set()
	);
	...
	const subscribeExperiencias = useCallback((callback: () => void) => {
		setExperienciasListeners((prev) => new Set([...prev, callback]));
		return () => {
			setExperienciasListeners((prev) => {
				const next = new Set(prev);
				next.delete(callback);
				return next;
			});
